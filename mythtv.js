...
function MythTv() {
	...
	function getRequest(url, port, callback) {
		var options = {
		host: frontendIp,
		port: port,
		path: url,
	...
	this.stop = function(callback) {
		...
		getRequest("/Frontend/SendAction?Action=STOPPLAYBACK", 6547, function(statusCode, results){
	...
	this.pause = function(callback) {
		...
		getRequest("/Frontend/SendAction?Action=PAUSE", 6547, function(statusCode, results){
			callback(statusCode === 200);
		});
	};
	...
	this.resume = function(callback) {
		...
		getRequest("/Frontend/SendAction?Action=PLAY", 6547, function(statusCode, results){
		callback(statusCode === 200);
		});
	};
			...
			if (statusCode == 200) {
				...
				var foundMatch = results.ProgramList.Programs.some(function(program, index){
					if ( program.Title.toLowerCase() === show.toLowerCase()) {
						...
						getRequest("/Frontend/PlayRecording?ChanId=" + program.Channel.ChanId + "&StartTime=" + program.StartTime, 6547, 
	...
	this.recordedListing = function(callback) {
		...
		getRequest("/Dvr/GetRecordedList?StartIndex=1&Count=100&Descending=true", 6544, callback);
	}
}
